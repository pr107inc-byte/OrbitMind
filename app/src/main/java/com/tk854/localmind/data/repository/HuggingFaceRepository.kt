...
import com.tk854.localmind.core.storage.PersistentModelStorageManager
...
import com.tk854.localmind.data.remote.HuggingFaceApi
import com.tk854.localmind.domain.model.HuggingFaceModelInfo
import com.tk854.localmind.domain.model.ModelCatalogItem
import com.tk854.localmind.domain.model.ModelCategory
import com.tk854.localmind.worker.DownloadModelWorker
...
class HuggingFaceRepository @Inject constructor(
    ...
    suspend fun searchHubModels(
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (com.tk854.localmind.BuildConfig.DEBUG) android.util.Log.w("HuggingFaceRepo", "error", e)
                emptyList()
            }
        }
    }

    private fun mapResponseToInfo(response: com.tk854.localmind.data.remote.HFModelResponse): HuggingFaceModelInfo {
     ...
     */
    suspend fun getActiveModel(): com.tk854.localmind.data.local.entity.ModelEntity? {
        return withContext(Dispatchers.IO) {
        ...
        workInfosFlow.collect { workInfos ->
            ...
            when (workInfo.state) {
                androidx.work.WorkInfo.State.RUNNING -> {
                    val progress = workInfo.progress.getInt(com.tk854.localmind.worker.DownloadModelWorker.KEY_PROGRESS, 0)
                    val downloaded = workInfo.progress.getLong(com.tk854.localmind.worker.DownloadModelWorker.KEY_DOWNLOADED_BYTES, 0L)
                    val total = workInfo.progress.getLong(com.tk854.localmind.worker.DownloadModelWorker.KEY_TOTAL_BYTES, 0L)
                    val speed = workInfo.progress.getLong(com.tk854.localmind.worker.DownloadModelWorker.KEY_SPEED_BPS, 0L)
                    val eta = workInfo.progress.getLong(com.tk854.localmind.worker.DownloadModelWorker.KEY_ETA_SECONDS, -1L)
                ...
                androidx.work.WorkInfo.State.FAILED -> {
                    val error = workInfo.outputData.getString(com.tk854.localmind.worker.DownloadModelWorker.KEY_ERROR_MESSAGE) ?: "Unknown error"
