...
class DocumentHelper(private val context: Context) {
    ...
    fun extractTextFromUri(uri: Uri): String? {
        return try {
            ...
            if (com.tk854.localmind.BuildConfig.DEBUG) android.util.Log.w("DocumentHelper", "extractText failed", e)
            null
        }
    }
    ...
    private fun extractTextFromPdf(inputStream: InputStream?): String? {
        ...
        return try {
            ...
            if (com.tk854.localmind.BuildConfig.DEBUG) android.util.Log.w("DocumentHelper", "extractTextFromPdf failed", e)
            null
        }
    }
}
