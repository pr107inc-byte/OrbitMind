package com.tk854.localmind.core.utils

import android.content.Context
import android.opengl.EGL14
import android.opengl.EGLConfig
import android.util.Log
import com.tk854.localmind.BuildConfig
 ...
 * GPU blindly enable na ho â€” jo crash aur incorrect results deta tha.
 */
data class GpuCapability(
    ...
    val isSupported: Boolean = hasVulkanFeature && !gpuRendererString.lowercase().let { 
        it.contains("software") || it.contains("swiftshader") || it.contains("llvmpipe") 
    }
) {
    fun toLogString(): String =
        "GPU=$gpuRendererString | Adreno=$isAdrenoGpu | VulkanFeature=$hasVulkanFeature | i8mm=$hasI8mm | dotprod=$hasDotprod | supported=$isSupported"
}
...
class GpuCapabilityChecker @Inject constructor(
    ...
    /**
     * GPU support check.
     * Allow any hardware GPU that supports Vulkan.
     */
    fun checkGpuSupport(): GpuCapability {
        ...
        val hasVulkanFeature = context.packageManager.hasSystemFeature(
        ...
        val capability = GpuCapability(
            ...
            hasVulkanFeature = hasVulkanFeature
        )
