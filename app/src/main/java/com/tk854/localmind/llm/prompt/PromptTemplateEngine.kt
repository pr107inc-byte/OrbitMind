package com.tk854.localmind.llm.prompt
...
class PromptTemplateEngine @Inject constructor() {
    fun buildPrompt(
        ...
        when (family) {
            ...
            // DeepSeek R1 / QwQ: add <think> prefix to trigger reasoning mode
            // Without this prefix, model skips <think> block and answers directly
            ...
            TemplateFamily.QWQ -> {
                ...
                appendChatMlMessage(builder, "user", currentUserInput.trim())
                // <think> prefix forces reasoning when thinking is enabled
                // When disabled, logit bias in C++ also blocks <think> token
                if (addGenPrompt && enableThinking) builder.append("<|im_start|>assistant\n<think>\n")
                else if (addGenPrompt) builder.append("<|im_start|>assistant\n")
            }

            TemplateFamily.SMOLVLM -> {
                if (bosEnabled) builder.append("<|im_start|>")
                if (systemPrompt.isNotBlank()) appendSmolVlmMessage(builder, "system", systemPrompt)
                history.forEach { msg ->
                    appendSmolVlmMessage(builder, msg.role.toTemplateRole(), msg.content)
                }
                appendSmolVlmMessage(builder, "user", currentUserInput.trim())
                if (addGenPrompt) builder.append("Assistant:")
            }
    ...
    private fun appendSmolVlmMessage(builder: StringBuilder, role: String, content: String) {
        val trimmed = content.trim()
        if (trimmed.isBlank()) return

        when (role.lowercase()) {
            "system" -> builder.append(trimmed).append("\n\n")
            "user" -> builder.append("User: ").append(trimmed).append("<end_of_utterance>\n")
            else -> builder.append("Assistant: ").append(trimmed).append("<end_of_utterance>\n")
        }
    }
