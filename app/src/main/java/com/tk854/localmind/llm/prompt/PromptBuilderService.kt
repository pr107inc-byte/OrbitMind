...
data class PromptBuildRequest(
    ...
    // POCKETPAL FIX: Dynamic EOS detection â€” model file path pass karo
    // taaki GGUF metadata se actual EOS token detect ho sake.
    val modelPath: String? = null,
    // FIX: Reasoning models (<think> block) generation time pe 500-2000 extra tokens use karte hain.
    // Token budget mein inka hissa reserve karna zaroori hai warna main response cut ho jaata hai.
...
class PromptBuilderService @Inject constructor(
    ...
    fun buildPrompt(request: PromptBuildRequest): PromptBuildOutput {
        ...
        // POCKETPAL FIX: Dynamic EOS detection â€” model path se GGUF metadata padho.
        ...
        val summaryMessage = if (!request.currentSummary.isNullOrBlank()) {
            Message(
                id = "summary",
                conversationId = "",
                role = com.tk854.localmind.domain.model.MessageRole.ASSISTANT, // Treat as context/memo
        ...
        // Oldest messages drop karo jab budget khatam ho â€” NEWEST messages preserve karo.
        ...
        fun renderPrompt(history: List<Message>, userInput: String): String {
            return promptTemplateEngine.buildPrompt(
                ...
                enableThinking = request.isReasoningModel
            )
        }
